 ...
 * @author jtapia
 */
public class Nprofesor extends GenericForwardComposer {
    ...
    Integer op = 0;
    String mensaje = "";
    ArrayList<Profesor> lista = consu.ConsultaProfesor("cursos", "select * from profesor WHERE estado='ACTIVO'  order by nombre_profesor");
    ArrayList<String> validar = new ArrayList<String>();
    //////////////////////

    public void onCreate$nprofesor() {
        inHabilitar(true);
    }
    ...
    public int validarCampos() {
        ...
        if (Validacion.Cedula(cedula.getText()) == false) {
            ...
            if (consu.ConsultaProfesor("cursos", "select * from profesor WHERE estado='ACTIVO' and cedula='"
    ...
    public void record(String cadena) {
        ...
        try {
            ...
            String val2 = "'" + usuario + "','" + ("P-" + lista2.get(pos).getIdusuario() + cedula.getText()) + "'";  
            dao.EditarNM("cursos", "usuario,clave", val2, "usuarios", "id_usuario",lista2.get(pos).getIdusuario() );
            String camp = "cedula,nombre_profesor,apellido,telefono,direccion,correo,fechas,nombre_rol,id_rol,nombre_usuario,estado";
            val = "'" + cedula.getText() + "','" + nombre.getText() + "','" + apellido.getText()
                    + "','" + telefono.getText() + "','" + direccion.getText() + "','" + correo.getText() + "','" + Fecha.Fecha()
                    + "','" + "PROFESOR" + "','" + 3 + "','" + usuario + "','" + "ACTIVO" + "'";
            dao.Guardar("cursos", "profesor", camp, val);
         lista2.clear();
        } catch (Exception ex) {
