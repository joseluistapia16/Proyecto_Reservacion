 ...
 * @author jtapia
 */
public class eliminarECurso extends GenericForwardComposer {
    ...
    public void delete() {
        ...
        String camp = "id_curso,id_alumno,estado";
        String val = "'" + lista.get(posi).getId_curso() + "','" + cal + "','" + "INACTIVO" + "'";
        try {
            dao.Editar("cursos", camp, val, "CursosEstudiante", "id_cursoestudiante", id_cursoE);
        } catch (Exception ex) {
            Logger.getLogger(eliminarECurso.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
