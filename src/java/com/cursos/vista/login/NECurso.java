 ...
 * @author jtapia
 */
public class NECurso extends GenericForwardComposer {
 ...
 public void onOpen$nombreestudiante() { 
    ...
    public void record() {
        ...
        String val = "'" + lista.get(posi).getId_curso() + "','" + cal  + "','" + "ACTIVO" + "','" + nombrecurso.getText()+ "','" + cedulaE 
           +     "','" + lista.get(posi).getId_profesor()+"'"; 
        System.out.println(val+" veremos "+camp);
        try {
            dao.Guardar("cursos", "cursosestudiante", camp, val);
        } catch (Exception ex) {
            Logger.getLogger(NECurso.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
